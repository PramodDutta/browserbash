...
function renderDetail(r) {
  ...
  if (!rec.length) rec.push(\`<div class="card"><span class="lbl">recording</span><p style="font-size:12px;color:var(--soft);margin:0">No recording. Re-run with <code>--record</code> to capture a screenshot (plus video + trace on the builtin engine).</p></div>\`);
  m.innerHTML = \`
    <span class="badge b-\${esc(r.status)}">\${esc(r.status)}</span>
