---
title: Test a Qwik City App With AI Browser Automation
description: "Learn how to test qwik city app with plain-English browser automation, deterministic checks, reusable logins, CI workflows, and honest model guidance."
date: 2026-08-14
category: guide
---

To test qwik city app well, you need to validate more than a successful build or a handful of component states. You need evidence that a reader or customer can load a real route, understand the rendered page, interact with routes, loaders, resumable actions, and delayed interactivity, and reach the expected result. BrowserBash lets you describe that outcome in plain English while an AI agent drives real Chrome or Chromium. Deterministic checks then turn important conditions into evidence rather than opinion.
...
## Why test qwik city app through user intent

Qwik resumes serialized application state instead of hydrating the whole page eagerly. Tests should therefore distinguish first-render content from the first interaction that resumes an event handler.

A unit test can tell you that a helper returns the right value. A framework test can confirm that a component receives expected data. Neither proves that the assembled deployment works in a browser with real routing, cookies, layout, accessible names, and client behavior. When you test qwik city app, the browser journey is the final contract.

Intent also survives many harmless implementation changes. "add an item to the cart and verify the cart count" describes a user goal. It does not care whether the control moved into a new component or acquired another generated class. The agent observes the page, chooses an action, and proceeds step by step without selectors or page objects.
...
browserbash run "Open http://localhost:3000/shop, confirm the page is about Shop, then add an item to the cart and verify the cart count"
browserbash run "Open https://preview.example.com/shop and verify the primary navigation works" --viewport 390x844
```
...
## Design a focused test qwik city app objective

Start with a risk, not a tour of the whole site. For Qwik City, high-value risks usually sit at rendering and interaction boundaries: a generated route is absent, a navigation item points to the wrong page, a form reaches the server but does not update the page, or an enhanced widget fails after the initial HTML appears.
...
For this guide, a good first journey is: open `/shop`, add an item to the cart and verify the cart count, then confirm the destination or updated state. Add a separate objective for a mobile viewport rather than asking one long agent run to cover desktop, mobile, keyboard use, errors, and every navigation item.
...
The same discipline applies to assertions. "Make sure it looks right" leaves too much room for judgment. "Verify the URL contains `/shop` and the 'Shop' heading is visible" identifies observable conditions. BrowserBash can compile supported Verify grammar into Playwright checks, so those exact statements do not ask a model to decide.
...
A representative test file can POST test data, use `Expect status 201, store $.id as 'recordId'`, open `/shop`, perform the user action, then use `Verify URL contains "/shop"` and `Verify 'Shop' heading visible`. That sequence separates setup, adaptive movement, and strict proof without hiding which layer failed.
...
When you test qwik city app, divide coverage by observable boundaries. First rendering asks whether the requested URL returns meaningful, navigable content. Routing asks whether links, deep URLs, back and forward navigation, and parameters land on the right page. Interaction asks whether the behavior layered onto that content actually works.
...
For client behavior, trigger the smallest meaningful interaction. Add an item to the cart and verify the cart count is better evidence than merely checking that a control exists. Also test refresh after reaching a stateful route. Framework routers can appear correct during client navigation while the server or static host returns a 404 on refresh.
...
Private Qwik City routes need repeatable authentication. BrowserBash can open a visible browser so you log in once, then save the Playwright storage state. Reuse the named profile across a run, test file, full suite, or monitor.

```bash
browserbash auth save site-editor --url https://preview.example.com/login
browserbash run "Open https://preview.example.com/shop, add an item to the cart and verify the cart count" --auth site-editor
...
## Scale Qwik City checks in CI

BrowserBash `run-all` is a memory-aware parallel orchestrator. It derives concurrency from real CPU and RAM, prioritizes previously failed and slow tests, and detects flaky behavior. That is useful when Qwik City coverage grows beyond a few smoke journeys.
...
## When to choose BrowserBash to test qwik city app
...
### Qwik City-specific regression checklist

For Qwik City, separate initial visibility from resumed interaction. Load a deep route and verify its heading before doing anything. Then trigger an action such as adding an item, submitting a form, or expanding a control, and verify the state produced after the handler resumes. A refresh after the action can reveal whether the result was persisted or existed only in memory.

Include one navigation through a Qwik City link and one direct request to the destination. Both should produce the same meaningful page, even though their loading paths differ. Keep the BrowserBash objective short enough that the model does not obscure timing-related evidence with unrelated actions. Lower-level tests remain the right place for loader outputs and action validation branches; the browser journey proves that routing, serialized state, resumption, and visible feedback meet at the user boundary.
...
### How do I test qwik city app without CSS selectors?
...
### Can BrowserBash test Qwik City with a local AI model?
...
### Does testmd v2 work with Ollama for Qwik City tests?
...
### Should BrowserBash replace Playwright in my Qwik City project?
