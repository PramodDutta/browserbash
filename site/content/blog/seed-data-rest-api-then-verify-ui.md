---
title: Seed Data With a REST API, Verify It in the UI
description: "Learn how to seed data REST API verify UI behavior in one BrowserBash testmd v2 file with deterministic setup and Playwright checks."
date: 2026-09-01
category: tutorial
---
A seed data rest api verify ui workflow is most useful when it removes setup clicks without hiding what the user actually experiences. The practical shape is simple: arrange a known customer record through an API, confirm that setup deterministically, keep one browser session alive, and inspect the resulting screen. BrowserBash testmd v2 puts those operations in one committable Markdown file, so the setup and the user-facing assertion travel together.
...
## Why seed data rest api verify ui belongs in one test

A browser-only test usually creates prerequisites by clicking through administration screens. That can be legitimate when those screens are the subject under test. It is wasteful when they are merely a slow route to customer records. Every extra navigation, modal, and form adds another page state that can fail before the scenario reaches its purpose.

An API-only test has the opposite blind spot. It can prove that https://app.example.test/api/customers accepts a request and returns a useful response, but it does not prove that the browser fetches, interprets, and renders that state. Serialization mistakes, stale caches, authorization boundaries, client-side filters, and inaccessible controls live after the endpoint.
...
That separation also gives reviewers a precise reason for every step and keeps setup failures from being mislabeled as browser regressions.

The main risk here is duplicate fixtures and stale shared data. Design the fixture so that repeated and parallel runs remain understandable. Prefer unique identifiers, an isolated test tenant, idempotent setup where the service supports it, and a cleanup policy owned by the environment. Do not let a convenient API shortcut bypass the exact security or business rule you intended to validate.

This pattern is strongest when the API is the supported setup boundary. Create the smallest record the screen needs, store a stable response field, and verify the user-facing representation rather than every JSON property.
...
## Build the seed data rest api verify ui file

The following example is deliberately small. Replace the example host with your test environment and pass secrets through variables rather than committing them. The setup creates or configures one customer record, stores one response value, then moves into the real browser.

```bash
cat > seed-data-rest-api-then-verify-ui_test.md <<'EOF'
---
version: 2
---
POST https://app.example.test/api/customers with body {"name":"Ada Lovelace","plan":"Pro"}
Expect status 200, store $.name as 'customerName'

Open https://app.example.test/customers
Open the customers page and filter for Ada Lovelace
Verify text 'Ada Lovelace' is visible
EOF

browserbash testmd seed-data-rest-api-then-verify-ui_test.md
```
...
A weak assertion proves only that a page loaded. A useful assertion connects the seeded state to a visible user outcome. For this scenario, verify text 'ada lovelace' is visible is the first check, but it may not be sufficient by itself. Add a URL, heading, control, or count assertion when that extra condition distinguishes the intended screen from a generic success message.
...
Stored value equality is useful at the API boundary. If you stored `customerName`, compare it against the expected setup value when the grammar and returned type fit the scenario. Do not imply that a stored JSON value automatically becomes a browser cookie, localStorage entry, or form value. Ground truth about the handoff matters more than a compact demo.
...
```bash
browserbash testmd seed-data-rest-api-then-verify-ui_test.md --agent
...
### How do I seed test data through a REST API?
...
### Can BrowserBash verify API data in the UI?
...
### Does testmd v2 work with Ollama?
...
### Should UI tests delete seeded records?
