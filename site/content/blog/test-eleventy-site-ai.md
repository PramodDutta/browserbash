---
title: Test an Eleventy Static Site in Plain English
description: "Learn how to test eleventy site with plain-English browser automation, deterministic checks, reusable logins, CI workflows, and honest model guidance."
date: 2026-08-16
category: guide
---

To test eleventy site well, you need to validate more than a successful build or a handful of component states. You need evidence that a reader or customer can load a real route, understand the rendered page, interact with pre-rendered pages, collections, navigation, forms, and progressive enhancement, and reach the expected result. BrowserBash lets you describe that outcome in plain English while an AI agent drives real Chrome or Chromium. Deterministic checks then turn important conditions into evidence rather than opinion.
...
## Why test eleventy site through user intent

Eleventy produces static output, but the delivered site can still fail through broken links, missing generated pages, bad asset paths, or a progressive enhancement script. Browser testing covers the deployed artifact rather than only the build.

A unit test can tell you that a helper returns the right value. A framework test can confirm that a component receives expected data. Neither proves that the assembled deployment works in a browser with real routing, cookies, layout, accessible names, and client behavior. When you test eleventy site, the browser journey is the final contract.

Intent also survives many harmless implementation changes. "filter the article list and open the first matching article" describes a user goal. It does not care whether the control moved into a new component or acquired another generated class. The agent observes the page, chooses an action, and proceeds step by step without selectors or page objects.
...
browserbash run "Open http://localhost:3000/articles, confirm the page is about Articles, then filter the article list and open the first matching article"
browserbash run "Open https://preview.example.com/articles and verify the primary navigation works" --viewport 390x844
```
...
The same discipline applies to assertions. "Make sure it looks right" leaves too much room for judgment. "Verify the URL contains `/articles` and the 'Articles' heading is visible" identifies observable conditions. BrowserBash can compile supported Verify grammar into Playwright checks, so those exact statements do not ask a model to decide.
...
A representative test file can POST test data, use `Expect status 201, store $.id as 'recordId'`, open `/articles`, perform the user action, then use `Verify URL contains "/articles"` and `Verify 'Articles' heading visible`. That sequence separates setup, adaptive movement, and strict proof without hiding which layer failed.
...
When you test eleventy site, divide coverage by observable boundaries. First rendering asks whether the requested URL returns meaningful, navigable content. Routing asks whether links, deep URLs, back and forward navigation, and parameters land on the right page. Interaction asks whether the behavior layered onto that content actually works.
...
For client behavior, trigger the smallest meaningful interaction. Filter the article list and open the first matching article is better evidence than merely checking that a control exists. Also test refresh after reaching a stateful route. Framework routers can appear correct during client navigation while the server or static host returns a 404 on refresh.
...
Private Eleventy routes need repeatable authentication. BrowserBash can open a visible browser so you log in once, then save the Playwright storage state. Reuse the named profile across a run, test file, full suite, or monitor.

```bash
browserbash auth save site-editor --url https://preview.example.com/login
browserbash run "Open https://preview.example.com/articles, filter the article list and open the first matching article" --auth site-editor
...
## Scale Eleventy checks in CI

BrowserBash `run-all` is a memory-aware parallel orchestrator. It derives concurrency from real CPU and RAM, prioritizes previously failed and slow tests, and detects flaky behavior. That is useful when Eleventy coverage grows beyond a few smoke journeys.
...
## When to choose BrowserBash to test eleventy site
...
### Eleventy-specific regression checklist

For Eleventy, start with generated route integrity. Open a recent collection item directly, follow its previous or next link, and return through a taxonomy or index page. Verify headings and URLs at each meaningful boundary. This catches permalink, collection ordering, base-path, and template-link problems that a successful build alone cannot expose.

Then identify every progressive enhancement that users depend on, such as filtering, disclosure navigation, a contact form, or copied code. Give each behavior a short focused journey and verify the visible result. Also decide what should remain usable when enhancement fails, and cover that baseline with a separate code-first check if disabling JavaScript is important. BrowserBash validates the shipped experience in normal Chrome, while link checkers and HTML validators still provide cheaper, broader coverage for thousands of static references and markup rules.
...
### How do I test eleventy site without CSS selectors?
...
### Can BrowserBash test Eleventy with a local AI model?
...
### Does testmd v2 work with Ollama for Eleventy tests?
...
### Should BrowserBash replace Playwright in my Eleventy project?
