...
Keep Node.js, the BrowserBash CLI, test files, and ideally Chrome in the same WSL distribution to reduce path and networking surprises. Install with npm inside Linux, not by accidentally invoking a Windows global npm installation. If Chrome runs on Windows, bind DevTools conservatively and resolve the host address from WSL rather than assuming localhost behavior.
...
Reproducibility is a bill of materials plus controlled state. It is not simply "we used a container" or "the job ran on the same runner label." For ai tests wsl, capture enough context to recreate the browser behavior without guessing.

Store the repository in the WSL filesystem when Linux tools perform most file operations. Crossing into mounted Windows drives can alter performance and permission behavior. Pin the WSL distribution, Node version, Chrome version, locale, and test dependencies for repeatable team setup.
...
## Diagnose ai tests wsl failures by layer
...
Print which node, npm, browserbash, and Chrome binaries are being used. Check file ownership, DISPLAY or WSLg state, host resolution, firewall rules, and the DevTools version endpoint before debugging the objective.
...
Mixing Windows and Linux binaries, paths, permissions, and global npm prefixes is the failure pattern to put near the top of your runbook. Also define timeouts at each layer. Allocation timeout, endpoint readiness timeout, objective timeout, and suite timeout answer different questions. A single thirty-minute job timeout tells you only that the cleanup finally occurred.
...
## When to choose WSL2, and when not to

Choose this pattern when your team develops on Windows but wants a consistent Linux shell and package environment. It is especially useful when the organization wants plain-English browser validation without reintroducing selector-heavy page objects solely to connect to centralized infrastructure. The browser remains real, results remain structured, and deterministic assertions can carry release-gate semantics.

Do not choose it merely because remote browser architecture sounds more mature. A local provider is the simplest default when one runner and one Chrome satisfy the trust, capacity, and reproducibility requirements. Avoid this pattern when native Windows tooling already meets the need or corporate policy blocks Chrome and WSL integration. Complexity must buy isolation, capacity, locality, or repeatability that you can name and measure.
...
Finally, test cleanup by killing the job mid-run. The browser allocation should expire, credentials should become useless, temporary data should be bounded, and the next run should receive a fresh profile. Document the owner for browser images, endpoint security, model keys, test data, and triage. ai tests wsl becomes dependable when every boundary has an owner and evidence.

## FAQ

### Can BrowserBash run ai tests wsl without CSS selectors?
...
### Does WSL2 require a paid AI model?
