---
title: UI Checks That Catch Broken Access Control
description: "Build a broken access control UI test that checks direct URLs, hidden actions, role boundaries, denial states, and real-browser evidence."
date: 2026-11-09
category: security
---
broken access control ui test is easy to describe and surprisingly easy to test badly. A shallow check proves that a page exists. A useful security test proves what a real user can do, what the interface reveals, and what remains impossible after the state changes. For broken access control in the UI, the target is concrete: a lower-privilege identity cannot view, invoke, or infer administrator-only screens and actions. That requires a real browser, controlled identities, explicit negative paths, and evidence that distinguishes product behavior from an agent's interpretation.
...
## Why broken access control ui test needs a real browser
...
For this scenario, the expected outcome is that restricted controls stay unavailable, direct routes deny access before protected data renders, backend actions reject the lower role, and the authorized role still works. Each clause deserves its own observation. Combining them into one broad instruction can produce an answer that sounds plausible but is hard to audit. Break the journey into setup, action, state transition, denial, recovery, and cleanup. The agent can navigate by intent, while deterministic checks anchor important claims.

The main operational hazard is equally concrete: checking only whether an admin button is hidden creates false confidence. Security automation should use test-only identities and isolated data. Put cleanup in the design before scheduling the test. If a scenario changes authentication state, assume retries and parallel CI workers can collide unless every run owns distinct data.
...
## Threat model and acceptance criteria for broken access control ui test

Start with the abuse case, not the happy path. Write down the actor, starting privileges, protected asset, attempted action, expected denial, and evidence of denial. The actors here are a lower-privilege user and an administrator saved auth profile. The protected asset may be a session, form submission, privileged page, account state, or sensitive action. State exactly what must never become available.

Translate policy into observable acceptance criteria. Avoid phrases such as "secure enough" or "behaves correctly." Prefer statements such as "a lower role must not retrieve admin data even briefly before a client-side redirect." Add positive criteria too, because a control that blocks everyone is not correct. An authorized user should still complete the intended task, and a denied user should receive a useful next step without internal details.

Create a permissions matrix and dedicated identities. List protected routes and high-impact actions, including paths that are not linked from normal navigation. Freeze those assumptions in test data or environment documentation. If policy differs by tenant, geography, plan, or risk score, parameterize the cases rather than treating one result as universal.
...
browserbash run "At https://staging.example.test, sign in as the lower role, inspect menus, enter protected URLs directly, attempt relevant actions, then compare with an authorized administrator session. Confirm that restricted controls stay unavailable, direct routes deny access before protected data renders, backend actions reject the lower role, and the authorized role still works."
browserbash run "At https://staging.example.test, verify that a lower role must not retrieve admin data even briefly before a client-side redirect." --agent
```
...
browserbash run "Open the protected area and sign in as the lower role, inspect menus, enter protected URLs directly, attempt relevant actions, then compare with an authorized administrator session" --auth security-user
browserbash monitor "Verify that a lower role must not retrieve admin data even briefly before a client-side redirect" --auth security-user --every 10m --notify https://hooks.example.test/security
```
...
A credible broken access control ui test suite covers four families of behavior. The positive path proves an allowed user can complete the action. The negative path proves a lower role must not retrieve admin data even briefly before a client-side redirect. The boundary path exercises the exact transition where policy changes. The recovery path proves the user can return to a safe, supported state without bypassing the control.
...
Choose this browser workflow to exercise realistic navigation, client routing, and role-specific screens. Add API authorization tests, object-level access tests, and manual security review because broken access control extends beyond what a UI can reveal.
...
### What is a broken access control UI test?
...
### Is a hidden admin link enough to prevent escalation?
...
### How do I test direct admin URLs as a normal user?
...
### Can AI browser tests replace authorization API tests?
