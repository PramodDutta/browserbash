---
title: Test Metabase Dashboards With AI Browser Automation
description: "Learn how to test metabase dashboards with plain-English browser automation, deterministic checks, practical CI patterns, and honest chart limits."
date: 2026-10-22
category: guide
---
If you need to test metabase dashboards, the hard part is rarely opening the page. The hard part is expressing what a useful dashboard session means without coupling the test to generated selectors, transient markup, or one exact data point. BrowserBash lets you write that intent in plain English, drives a real Chrome or Chromium browser step by step, and returns a deterministic verdict with structured results.

That approach fits Metabase because users interact with questions, dashboard cards, date filters, category filters, and drill-through links. Those controls are meaningful to an analyst, but their DOM structure can be noisy and can change across upgrades. Intent-level automation gives you a readable outer layer. It does not remove the need for API, component, accessibility, or data-quality tests. It gives those layers an end-to-end companion that answers a narrower question: can a real user reach and operate the expected experience?
...
## Why teams test metabase dashboards at the browser level
...
A browser test should therefore focus on user-observable contracts. For Metabase, that means open a dashboard, apply a date or category filter, observe connected questions update, and inspect the resulting table or headline value. These are not implementation details. They are the workflow a product owner, analyst, support engineer, or operator recognizes.
...
## A first plain-English test for Metabase
...
browserbash run "Open https://analytics.example.com/dashboard/12; Confirm the Revenue dashboard is visible; Set Region to West; Apply the filter; Verify text 'West' is visible; Confirm the Orders question shows results"
```
...
For this subject, the main boundary is clear: a Metabase card can be backed by SQL, the query builder, or a visualization, and a UI test sees the rendered outcome rather than proving the underlying query is analytically correct. BrowserBash should not be presented as a pixel-diff engine. It can operate real browser controls and inspect states exposed to the page, but a canvas does not automatically expose each plotted mark as an element. Even SVG charts can use generated nodes and volatile labels that are poor long-term contracts.
...
BrowserBash is a good fit for Metabase teams that want business-readable coverage across shared dashboards, questions, filters, and drill paths. It is especially useful when non-specialists need to review the workflow, selectors are costly to maintain, and an AI coding agent needs a structured browser verdict.
...
### Can BrowserBash test metabase dashboards without CSS selectors?
