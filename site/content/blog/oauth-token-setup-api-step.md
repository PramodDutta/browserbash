---
title: Set Up an OAuth Token With an API Step, Then Test the UI
description: "Create an OAuth token API step test with deterministic token setup, saved browser login reuse, and UI assertions in BrowserBash testmd v2."
date: 2026-09-03
category: tutorial
---
A oauth token api step test workflow is most useful when it removes setup clicks without hiding what the user actually experiences. The practical shape is simple: arrange a known access token through an API, confirm that setup deterministically, keep one browser session alive, and inspect the resulting screen. BrowserBash testmd v2 puts those operations in one committable Markdown file, so the setup and the user-facing assertion travel together.
...
## Why oauth token api step test belongs in one test

A browser-only test usually creates prerequisites by clicking through administration screens. That can be legitimate when those screens are the subject under test. It is wasteful when they are merely a slow route to access tokens. Every extra navigation, modal, and form adds another page state that can fail before the scenario reaches its purpose.

An API-only test has the opposite blind spot. It can prove that https://auth.example.test/oauth/token accepts a request and returns a useful response, but it does not prove that the browser fetches, interprets, and renders that state. Serialization mistakes, stale caches, authorization boundaries, client-side filters, and inaccessible controls live after the endpoint.
...
The main risk here is confusing an API bearer token with a browser cookie session. Design the fixture so that repeated and parallel runs remain understandable. Prefer unique identifiers, an isolated test tenant, idempotent setup where the service supports it, and a cleanup policy owned by the environment. Do not let a convenient API shortcut bypass the exact security or business rule you intended to validate.

Do not assume that storing an OAuth response field automatically injects it into Chrome storage or cookies. BrowserBash can store a response value for deterministic comparisons, while browser authentication should use a supported application flow or a saved login profile. That separation avoids pretending two different credential mechanisms are interchangeable.
...
## Build the oauth token api step test file

The following example is deliberately small. Replace the example host with your test environment and pass secrets through variables rather than committing them. The setup creates or configures one access token, stores one response value, then moves into the real browser.

```bash
cat > oauth-token-setup-api-step_test.md <<'EOF'
---
version: 2
auth: qa-user
---
POST https://auth.example.test/oauth/token with body {"grant_type":"client_credentials","client_id":"{{client_id}}","client_secret":"{{client_secret}}"}
Expect status 200, store $.access_token as 'accessToken'

Open https://app.example.test/account
Open the account page using the saved authenticated browser profile
Verify 'Account' heading is visible
EOF

browserbash testmd oauth-token-setup-api-step_test.md
```
...
A weak assertion proves only that a page loaded. A useful assertion connects the seeded state to a visible user outcome. For this scenario, verify 'account' heading is visible is the first check, but it may not be sufficient by itself. Add a URL, heading, control, or count assertion when that extra condition distinguishes the intended screen from a generic success message.
...
Stored value equality is useful at the API boundary. If you stored `accessToken`, compare it against the expected setup value when the grammar and returned type fit the scenario. Do not imply that a stored JSON value automatically becomes a browser cookie, localStorage entry, or form value. Ground truth about the handoff matters more than a compact demo.
...
```bash
browserbash testmd oauth-token-setup-api-step_test.md --agent
...
### How can I fetch an OAuth token before a UI test?
...
### Does a stored OAuth token automatically log in Chrome?
...
### How should secrets be handled in BrowserBash tests?
...
### When should I use a saved login instead of OAuth setup?
