---
title: Lightweight Contract Testing Between UI and API
description: "Use lightweight contract testing UI API checks to pin status and key response values before verifying the same behavior in a real browser."
date: 2026-09-08
category: tutorial
---
A contract testing ui api workflow is most useful when it removes setup clicks without hiding what the user actually experiences. The practical shape is simple: arrange a known feature configuration through an API, confirm that setup deterministically, keep one browser session alive, and inspect the resulting screen. BrowserBash testmd v2 puts those operations in one committable Markdown file, so the setup and the user-facing assertion travel together.
...
## Why contract testing ui api belongs in one test

A browser-only test usually creates prerequisites by clicking through administration screens. That can be legitimate when those screens are the subject under test. It is wasteful when they are merely a slow route to feature configurations. Every extra navigation, modal, and form adds another page state that can fail before the scenario reaches its purpose.

An API-only test has the opposite blind spot. It can prove that https://app.example.test/api/dashboard-config accepts a request and returns a useful response, but it does not prove that the browser fetches, interprets, and renders that state. Serialization mistakes, stale caches, authorization boundaries, client-side filters, and inaccessible controls live after the endpoint.
...
That separation gives reviewers a precise reason for every step and prevents setup failures from being mislabeled as browser regressions.

The main risk here is turning a lightweight consumer check into a brittle full-schema duplicate. Design the fixture so that repeated and parallel runs remain understandable. Prefer unique identifiers, an isolated test tenant, idempotent setup where the service supports it, and a cleanup policy owned by the environment. Do not let a convenient API shortcut bypass the exact security or business rule you intended to validate.

This is not a replacement for a schema contract suite. It is a narrow executable pin on the fields the UI actually consumes. Check status, store one or two critical values, and then prove the browser renders the capability. Leave exhaustive schema compatibility to dedicated contract tooling.
...
## Build the contract testing ui api file

The following example is deliberately small. Replace the example host with your test environment and pass secrets through variables rather than committing them. The setup creates or configures one feature configuration, stores one response value, then moves into the real browser.

```bash
cat > contract-test-ui-with-api-steps_test.md <<'EOF'
---
version: 2
---
POST https://app.example.test/api/dashboard-config with body {"tenant":"qa","widget":"revenue","enabled":true}
Expect status 200, store $.widget as 'widgetName'

Open https://app.example.test/dashboard
Open the dashboard for the qa tenant
Verify 'Revenue' heading is visible
EOF

browserbash testmd contract-test-ui-with-api-steps_test.md
```
...
A weak assertion proves only that a page loaded. A useful assertion connects the seeded state to a visible user outcome. For this scenario, verify 'revenue' heading is visible is the first check, but it may not be sufficient by itself. Add a URL, heading, control, or count assertion when that extra condition distinguishes the intended screen from a generic success message.
...
Stored value equality is useful at the API boundary. If you stored `widgetName`, compare it against the expected setup value when the grammar and returned type fit the scenario. Do not imply that a stored JSON value automatically becomes a browser cookie, localStorage entry, or form value. Ground truth about the handoff matters more than a compact demo.
...
```bash
browserbash testmd contract-test-ui-with-api-steps_test.md --agent
...
### What is lightweight UI API contract testing?
...
### Can BrowserBash replace Pact or schema validation?
...
### Which API fields should a UI contract test pin?
...
### Why combine API status checks with browser verification?
