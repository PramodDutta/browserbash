---
title: Gate Spinnaker Deploys With AI Browser Tests
description: "Use AI browser tests Spinnaker pipelines can trust, with BrowserBash smoke suites, structured verdicts, cost limits, and pre-promotion gates."
date: 2026-09-15
category: ci
---

ai browser tests spinnaker integration works best when you treat browser validation as a release contract, not as a loose shell command. Spinnaker supplies the orchestration boundary, while BrowserBash supplies a real Chrome or Chromium session, a deterministic verdict, and structured evidence that another system can read. That separation is useful for teams adding AI-driven navigation without giving up predictable CI behavior.
...
## Why ai browser tests spinnaker belong in the delivery path
...
The orchestrator's job is narrower. Spinnaker should provide source, configuration, secrets, network reachability, a browser-capable runtime, and durable artifacts. BrowserBash should own browser actions and verdict semantics. Spinnaker should consume the external job status as a gate and keep the richer BrowserBash artifacts at the job URL for operators. This boundary keeps the pipeline understandable when a test finds a real regression, the browser cannot start, a budget is exhausted, or the objective times out.
...
## Build the Spinnaker execution contract

A reliable pipeline stage has four inputs: the checked-out `*_test.md` files, a reachable start URL, model configuration, and any required authentication profile. It has three outputs: process status, machine-readable events, and human-readable reports. Write those expectations down before adding YAML or SDK code.
...
The minimal pipeline stage contract below shows the important shape. Adapt image policy, workspace paths, and artifact publication to your organization.

```bash
: "Command run by the CI job Spinnaker invokes"
npm install -g browserbash-cli
browserbash run-all tests/smoke   --agent   --budget-usd 2.00 > browserbash.ndjson
case $? in
  0) exit 0 ;;
  1) echo "Product validation failed"; exit 1 ;;
  2) echo "Infrastructure or budget stop"; exit 2 ;;
  3) echo "Validation timed out"; exit 3 ;;
esac
```

Do not pipe through a command that masks the original status. If you need `tee`, capture the BrowserBash status explicitly under your shell's pipeline rules. Also avoid interpreting exit code 1 as broken infrastructure. A failed test is a successful validation operation that found the product in the wrong state. That distinction becomes especially important when Spinnaker drives retries or alert routing.
...
BrowserBash is Ollama-first. Its resolution order is local Ollama, then `ANTHROPIC_API_KEY`, then `OPENAI_API_KEY`, then OpenRouter. Local models require no API keys and keep prompts on your machine. That is attractive on a trusted Spinnaker worker, especially when the tested environment or data cannot leave a private network.
...
Replay cache is often the largest cost and speed win. After a green run, BrowserBash records actions. The next identical run replays them with zero model calls, and the agent returns only if the page changed. This is different from caching the entire Spinnaker task. BrowserBash replay still exercises the browser against the current target, while an orchestrator-level cache may simply restore old output. For deployment gates, keep the live task uncached unless your cache key includes every environmental fact that makes a past verdict safe to reuse.
...
A retry can be reasonable for a transient provider or network failure, but repeated product failures should remain failures. BrowserBash `run-all` already uses memory-aware concurrency derived from CPU and RAM, orders previously failed or slow tests first, and detects flaky behavior. Let that orchestrator control test parallelism before layering aggressive Spinnaker retries around the whole suite.
...
### How do I run ai browser tests spinnaker in CI?

Install BrowserBash in the Spinnaker execution environment, check out the Markdown tests, and run `browserbash run-all tests/browser --agent`. Preserve its exit code and upload NDJSON, JUnit, and Markdown results even when validation fails.

### Can ai browser tests spinnaker use a local model?
...
### How should Spinnaker interpret a failed BrowserBash test?
