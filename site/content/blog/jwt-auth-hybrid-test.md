---
title: Test a JWT-Protected UI With an API-Seeded Session
description: "Build a JWT auth hybrid test with deterministic API setup, an explicit browser session boundary, and gated UI verification in BrowserBash."
date: 2026-09-04
category: tutorial
---
A jwt auth hybrid test workflow is most useful when it removes setup clicks without hiding what the user actually experiences. The practical shape is simple: arrange a known JWT through an API, confirm that setup deterministically, keep one browser session alive, and inspect the resulting screen. BrowserBash testmd v2 puts those operations in one committable Markdown file, so the setup and the user-facing assertion travel together.
...
## Why jwt auth hybrid test belongs in one test

A browser-only test usually creates prerequisites by clicking through administration screens. That can be legitimate when those screens are the subject under test. It is wasteful when they are merely a slow route to JWTs. Every extra navigation, modal, and form adds another page state that can fail before the scenario reaches its purpose.

An API-only test has the opposite blind spot. It can prove that https://app.example.test/api/test-session accepts a request and returns a useful response, but it does not prove that the browser fetches, interprets, and renders that state. Serialization mistakes, stale caches, authorization boundaries, client-side filters, and inaccessible controls live after the endpoint.
...
The main risk here is mistaking token issuance for proof that the browser accepted the session. Design the fixture so that repeated and parallel runs remain understandable. Prefer unique identifiers, an isolated test tenant, idempotent setup where the service supports it, and a cleanup policy owned by the environment. Do not let a convenient API shortcut bypass the exact security or business rule you intended to validate.

A minted JWT is only one side of the boundary. Your application still needs a documented way to exchange or install it as browser session state. BrowserBash does not claim an automatic cookie or localStorage injection feature, so keep that handoff explicit, or reuse a saved login and treat token issuance as separate setup evidence.
...
## Build the jwt auth hybrid test file

The following example is deliberately small. Replace the example host with your test environment and pass secrets through variables rather than committing them. The setup creates or configures one JWT, stores one response value, then moves into the real browser.

```bash
cat > jwt-auth-hybrid-test_test.md <<'EOF'
---
version: 2
auth: qa-user
---
POST https://app.example.test/api/test-session with body {"user":"qa-admin","role":"admin"}
Expect status 200, store $.token as 'jwt'

Open https://app.example.test/admin
Open the admin page with the saved qa-admin browser profile
Verify 'Admin Console' heading is visible
EOF

browserbash testmd jwt-auth-hybrid-test_test.md
```
...
A weak assertion proves only that a page loaded. A useful assertion connects the seeded state to a visible user outcome. For this scenario, verify 'admin console' heading is visible is the first check, but it may not be sufficient by itself. Add a URL, heading, control, or count assertion when that extra condition distinguishes the intended screen from a generic success message.
...
Stored value equality is useful at the API boundary. If you stored `jwt`, compare it against the expected setup value when the grammar and returned type fit the scenario. Do not imply that a stored JSON value automatically becomes a browser cookie, localStorage entry, or form value. Ground truth about the handoff matters more than a compact demo.
...
```bash
browserbash testmd jwt-auth-hybrid-test_test.md --agent
...
### What is a JWT auth hybrid test?
...
### Can BrowserBash inject a JWT into localStorage?
...
### How do I verify a JWT-protected page?
...
### Is a saved auth profile safe for repeatable tests?
