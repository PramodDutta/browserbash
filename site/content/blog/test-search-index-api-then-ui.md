---
title: Seed a Search Index via API, Verify Results in the UI
description: "Learn to test search index API UI behavior with deterministic seeding, stable queries, and visible result checks in BrowserBash testmd v2."
date: 2026-09-10
category: tutorial
---
A test search index api ui workflow is most useful when it removes setup clicks without hiding what the user actually experiences. The practical shape is simple: arrange a known search document through an API, confirm that setup deterministically, keep one browser session alive, and inspect the resulting screen. BrowserBash testmd v2 puts those operations in one committable Markdown file, so the setup and the user-facing assertion travel together.
...
## Why test search index api ui belongs in one test

A browser-only test usually creates prerequisites by clicking through administration screens. That can be legitimate when those screens are the subject under test. It is wasteful when they are merely a slow route to search documents. Every extra navigation, modal, and form adds another page state that can fail before the scenario reaches its purpose.

An API-only test has the opposite blind spot. It can prove that https://app.example.test/api/test/search-documents accepts a request and returns a useful response, but it does not prove that the browser fetches, interprets, and renders that state. Serialization mistakes, stale caches, authorization boundaries, client-side filters, and inaccessible controls live after the endpoint.
...
The main risk here is index refresh latency, analyzers, ranking changes, and collisions with shared data. Design the fixture so that repeated and parallel runs remain understandable. Prefer unique identifiers, an isolated test tenant, idempotent setup where the service supports it, and a cleanup policy owned by the environment. Do not let a convenient API shortcut bypass the exact security or business rule you intended to validate.

Search needs a unique query term and a clear consistency strategy. Seed a document whose token cannot collide with ordinary content, then wait through a bounded application-supported refresh path. Assert inclusion and useful rendering before asserting exact rank, unless rank itself is the contract.
...
## Build the test search index api ui file

The following example is deliberately small. Replace the example host with your test environment and pass secrets through variables rather than committing them. The setup creates or configures one search document, stores one response value, then moves into the real browser.

```bash
cat > test-search-index-api-then-ui_test.md <<'EOF'
---
version: 2
---
POST https://app.example.test/api/test/search-documents with body {"id":"qa-orchid-731","title":"Orchid Runbook","body":"unique-browserbash-fixture"}
Expect status 200, store $.id as 'documentId'

Open https://app.example.test/search
Open search and query for unique-browserbash-fixture
Verify text 'Orchid Runbook' is visible
EOF

browserbash testmd test-search-index-api-then-ui_test.md
```
...
A weak assertion proves only that a page loaded. A useful assertion connects the seeded state to a visible user outcome. For this scenario, verify text 'orchid runbook' is visible is the first check, but it may not be sufficient by itself. Add a URL, heading, control, or count assertion when that extra condition distinguishes the intended screen from a generic success message.
...
Stored value equality is useful at the API boundary. If you stored `documentId`, compare it against the expected setup value when the grammar and returned type fit the scenario. Do not imply that a stored JSON value automatically becomes a browser cookie, localStorage entry, or form value. Ground truth about the handoff matters more than a compact demo.
...
```bash
browserbash testmd test-search-index-api-then-ui_test.md --agent
...
### How do I test search index API UI behavior?
...
### How should a test handle search indexing delay?
...
### Should a search UI test assert exact ranking?
...
### What makes a good search fixture?
