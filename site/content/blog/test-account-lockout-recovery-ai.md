---
title: Test Account Lockout and Recovery Flows
description: "Learn how to test account lockout recovery through the real UI, including safe messages, support paths, unlock states, and evidence."
date: 2026-11-07
category: security
---
test account lockout recovery is easy to describe and surprisingly easy to test badly. A shallow check proves that a page exists. A useful security test proves what a real user can do, what the interface reveals, and what remains impossible after the state changes. For account lockout recovery, the target is concrete: a locked user is denied safely and can follow the approved recovery or support path without weakening the lockout. That requires a real browser, controlled identities, explicit negative paths, and evidence that distinguishes product behavior from an agent's interpretation.
...
## Why test account lockout recovery needs a real browser
...
For this scenario, the expected outcome is that the UI communicates next steps without account enumeration, recovery proves the required identity, the lock clears only through policy, and audit-relevant actions are observable. Each clause deserves its own observation. Combining them into one broad instruction can produce an answer that sounds plausible but is hard to audit. Break the journey into setup, action, state transition, denial, recovery, and cleanup. The agent can navigate by intent, while deterministic checks anchor important claims.

The main operational hazard is equally concrete: poor cleanup can leave shared accounts locked or cause tests to depend on prior runs. Security automation should use test-only identities and isolated data. Put cleanup in the design before scheduling the test. If a scenario changes authentication state, assume retries and parallel CI workers can collide unless every run owns distinct data.
...
## Threat model and acceptance criteria for test account lockout recovery

Start with the abuse case, not the happy path. Write down the actor, starting privileges, protected asset, attempted action, expected denial, and evidence of denial. The actors here are a locked test user and a support or administrator recovery role. The protected asset may be a session, form submission, privileged page, account state, or sensitive action. State exactly what must never become available.

Translate policy into observable acceptance criteria. Avoid phrases such as "secure enough" or "behaves correctly." Prefer statements such as "changing a password alone must not clear a lock when policy requires separate approval." Add positive criteria too, because a control that blocks everyone is not correct. An authorized user should still complete the intended task, and a denied user should receive a useful next step without internal details.

Use a disposable account, a known lockout policy, and a supported reset mechanism. Separate automatic unlock, self-service recovery, administrator unlock, and support escalation. Freeze those assumptions in test data or environment documentation. If policy differs by tenant, geography, plan, or risk score, parameterize the cases rather than treating one result as universal.
...
browserbash run "At https://staging.example.test, place the account in a locked state, attempt login, follow the presented recovery path, complete the authorized unlock, and verify both old and recovered states. Confirm that the UI communicates next steps without account enumeration, recovery proves the required identity, the lock clears only through policy, and audit-relevant actions are observable."
browserbash run "At https://staging.example.test, verify that changing a password alone must not clear a lock when policy requires separate approval." --agent
```
...
browserbash run "Open the protected area and place the account in a locked state, attempt login, follow the presented recovery path, complete the authorized unlock, and verify both old and recovered states" --auth security-user
browserbash monitor "Verify that changing a password alone must not clear a lock when policy requires separate approval" --auth security-user --every 10m --notify https://hooks.example.test/security
```
...
A credible test account lockout recovery suite covers four families of behavior. The positive path proves an allowed user can complete the action. The negative path proves changing a password alone must not clear a lock when policy requires separate approval. The boundary path exercises the exact transition where policy changes. The recovery path proves the user can return to a safe, supported state without bypassing the control.
...
Choose browser automation when the lockout message and multi-page recovery journey are the acceptance target. Use API and security tests to validate counters, race behavior, audit events, and recovery-token enforcement.
...
### How do I test account lockout recovery?
...
### What is the difference between lockout and password reset?
...
### Should automated recovery tests contact real support?
...
### How do I reset test state after a lockout scenario?
