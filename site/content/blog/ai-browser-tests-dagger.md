...
The orchestrator's job is narrower. Dagger should provide source, configuration, secrets, network reachability, a browser-capable runtime, and durable artifacts. BrowserBash should own browser actions and verdict semantics. Dagger lets you model the validation container, secrets, service dependencies, and exported result directory in normal SDK code. This boundary keeps the pipeline understandable when a test finds a real regression, the browser cannot start, a budget is exhausted, or the objective times out.
...
The minimal Dagger SDK function below shows the important shape. Adapt image policy, workspace paths, and artifact publication to your organization.

```bash
// Inside a Dagger TypeScript function
const results = dag.directory()
const run = dag
  .container()
  .from("node:22")
  .withDirectory("/src", source)
  .withWorkdir("/src")
  .withExec(["npm", "install", "-g", "browserbash-cli"])
  .withExec(["sh", "-lc",
    "browserbash run-all tests/browser --agent > /tmp/events.ndjson"])
  .directory("/tmp")

await run.export("./browser-results")
```

Do not pipe through a command that masks the original status. If you need `tee`, capture the BrowserBash status explicitly under your shell's pipeline rules. Also avoid interpreting exit code 1 as broken infrastructure. A failed test is a successful validation operation that found the product in the wrong state. That distinction becomes especially important when Dagger drives retries or alert routing.
...
BrowserBash is Ollama-first. Its resolution order is local Ollama, then `ANTHROPIC_API_KEY`, then `OPENAI_API_KEY`, then OpenRouter. Local models require no API keys and keep prompts on your machine. That is attractive on a trusted Dagger worker, especially when the tested environment or data cannot leave a private network.
...
A retry can be reasonable for a transient provider or network failure, but repeated product failures should remain failures. BrowserBash `run-all` already uses memory-aware concurrency derived from CPU and RAM, orders previously failed or slow tests first, and detects flaky behavior. Let that orchestrator control test parallelism before layering aggressive Dagger retries around the whole suite.
...
### How do I run ai browser tests dagger in CI?

Install BrowserBash in the Dagger execution environment, check out the Markdown tests, and run `browserbash run-all tests/browser --agent`. Preserve its exit code and upload NDJSON, JUnit, and Markdown results even when validation fails.

### Can ai browser tests dagger use a local model?
...
### How should Dagger interpret a failed BrowserBash test?
