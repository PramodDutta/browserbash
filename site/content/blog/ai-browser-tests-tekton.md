...
The orchestrator's job is narrower. Tekton Pipelines should provide source, configuration, secrets, network reachability, a browser-capable runtime, and durable artifacts. BrowserBash should own browser actions and verdict semantics. Tekton workspaces keep the checked-out tests and generated JUnit or Result files available to later Tasks that publish reports. This boundary keeps the pipeline understandable when a test finds a real regression, the browser cannot start, a budget is exhausted, or the objective times out.
...
## Build the Tekton Pipelines execution contract

A reliable Task step has four inputs: the checked-out `*_test.md` files, a reachable start URL, model configuration, and any required authentication profile. It has three outputs: process status, machine-readable events, and human-readable reports. Write those expectations down before adding YAML or SDK code.
...
The minimal Task and Pipeline below shows the important shape. Adapt image policy, workspace paths, and artifact publication to your organization.
...
BrowserBash is Ollama-first. Its resolution order is local Ollama, then `ANTHROPIC_API_KEY`, then `OPENAI_API_KEY`, then OpenRouter. Local models require no API keys and keep prompts on your machine. That is attractive on a trusted Tekton Pipelines worker, especially when the tested environment or data cannot leave a private network.
...
Replay cache is often the largest cost and speed win. After a green run, BrowserBash records actions. The next identical run replays them with zero model calls, and the agent returns only if the page changed. This is different from caching the entire Tekton Pipelines task. BrowserBash replay still exercises the browser against the current target, while an orchestrator-level cache may simply restore old output. For deployment gates, keep the live task uncached unless your cache key includes every environmental fact that makes a past verdict safe to reuse.
...
A retry can be reasonable for a transient provider or network failure, but repeated product failures should remain failures. BrowserBash `run-all` already uses memory-aware concurrency derived from CPU and RAM, orders previously failed or slow tests first, and detects flaky behavior. Let that orchestrator control test parallelism before layering aggressive Tekton Pipelines retries around the whole suite.
...
Tekton workspaces keep the checked-out tests and generated JUnit or Result files available to later Tasks that publish reports. The best setup keeps logs streaming while artifacts remain durable. It also prevents concurrent tests from mutating the same account or record. Generate unique data through variables or testmd v2 API steps, and clean it up when the product supports safe cleanup.

## When to choose Tekton Pipelines and when not to

Choose Tekton Pipelines for ai browser tests tekton when the team already operates it, understands its security model, and can provide a browser-capable execution environment. Tekton is the better fit when your Kubernetes delivery system already standardizes reusable Tasks, Pipelines, Chains, and workspace contracts. Adding a second orchestrator only for browser tests usually creates more ownership than value.
...
### How do I run ai browser tests tekton in CI?

Install BrowserBash in the Tekton Pipelines execution environment, check out the Markdown tests, and run `browserbash run-all tests/browser --agent`. Preserve its exit code and upload NDJSON, JUnit, and Markdown results even when validation fails.

### Can ai browser tests tekton use a local model?
...
### How should Tekton Pipelines interpret a failed BrowserBash test?
